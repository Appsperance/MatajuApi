...
public class BookingController : ControllerBase
{
  ...
  public BookingController(IRepository<User> userTableRepo, IRepository<House> houseTableRepo, IRepository<Unit> unitTableRepo, IRepository<Booking> bookingRepo)
  {
  ...
  public IActionResult BookUnit([FromBody] BookUnitReqDto request)
  {
    ...
    Unit? availableUnit = _unitTableRepo.Find(u => u.HouseId == request.HouseId && u.Size == request.UnitSize && u.Status == UnitStatus.Available).FirstOrDefault();
    ...
    Booking booking = new Booking
                      {
                        UserId = request.UserId, UnitId = availableUnit.Id, RequestDate = requestDate, Type = BookingType.CheckIn, Charge = totalPrice, PaymentDate = null,
                        PaymentMethod = null, Status = BookingStatus.Pending, UserNote = request.UserNote
                      };
    ...
    return CreatedAtAction(nameof(BookUnit),
                           new { id = booking.Id },
                           new { Message = "예약이 성공적으로 생성되었습니다.", BookingId = booking.Id, TotalPrice = totalPrice, BookingDetails = booking });
  }
}